...
def exercise12(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    checkerboard_matrix = 1 - np.indices((2*n, 2*n)).sum(axis=0) % 2
    # ------ Place code above here /\ /\ /\ ------ 
...
def exercise13(n):
    '''
    Create a pandas Series, s, with n random integers between 0 and n, for n days starting from 1/1/2010 and plot the cumulative sum on a  chart. The data range should be an index. pd.date_range() may help. 
    ...
    s.cumsum().plot()
    plt.show() # <- not being tested so I commented it out. 
    # ------ Place code above here /\ /\ /\ ------ 
    return s
...
def exercise14(words):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df = pd.Series({word: len(word) for word in words})
    # ------ Place code above here /\ /\ /\ ------ 
    return df
...
class TestAssignment5(unittest.TestCase):
    def test_exercise15(self):
        ...
        df = exercise15()
        expected_pd_df = pd.DataFrame({'zip': [95838]}) 
        e = expected_pd_df.at[0,'zip']
        d = df.at[0, 'zip']
        self.assertEqual(d, e) 
    
    def test_exercise14(self):
        print('Testing exercise 14')
        expected_data = {"cat": 3,"frog": 4,"walrus": 6,"antelope": 8}
        expected_series = pd.Series(expected_data)
        df = exercise14(['cat','frog','walrus','antelope'])
        self.assertEqual(df.tolist(), expected_series.tolist())
