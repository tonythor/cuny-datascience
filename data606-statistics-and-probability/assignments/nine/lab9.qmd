...
_This was an observational study. It would be an experiment if the same instructor came in all frumpy dressed and not put together one semester, and then the next semester he came totally put together, which is not what happened. Yes I think it is possible. It might be better to infer beauty leads to higher course evaluations, but as is beauty definitely leads to differences in course evaluations._
...
boxplot_ethnicity <- ggplot(evals, aes(x = ethnicity, y = score, fill = ethnicity)) +
  geom_boxplot() +
  labs(title = "Scores by ethnicity", x = "Gender", y = "Score") +
  theme_minimal()

grid.arrange(boxplot_gender, boxplot_ethnicity, ncol = 2)
```

_Wow. So beautiful, white, and male all mean you're going to have higher ratings if you are an instructor. I guess teachers have the same problems we have, we all gotta hit the gym if we want the satisfaction scores._
