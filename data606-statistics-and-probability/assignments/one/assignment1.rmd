...
1. How would you describe it? (To ensure that your lab report is comprehensive,
   be sure to include the code needed to make the plot as well as your written
   interpretation.)

**Insert your answer here 1**

1. _Yes, there is an trend between years and girls being baptized._
1. _It's an upward trend, you can see it from the plot above, or you can see a rapidly rising slope if you overlay the same plot with a trend line._

```{r show-relationsip-upward-trend1}
ggp <- ggplot(data = arbuthnot, aes(x = year, y = girls)) + 
  geom_line()
...
**Insert your answer here 2**

__This proportion boys over time plot shows two things:__

1. _There is sampling bias toward boys. If all the points on the graph were exactly
on the dotted line, there would be and even 50/50 split between surveyed girls and 
boys. Because each year's ratio is above that 50/50 line, it means that every
year more boys were sampled. In some years, there were a considerably more boys than girls
sampled._

2. _If you put a trend line though the data points like in this chart, there is a slightly 
downward trend toward more equal sampling. Perhaps if the study continued for another
100 years it sampling might have reached equilibrium._

__Note__ ->  _From modern statistics we know there are slightly more boys than 
there are girls, about 1.05 to 1. This graph is not likely to ever be 50/50, but 
it should be closer than it is._ 

```{r echo=TRUE}
ggp <- ggplot(data = arbuthnot, aes(x = year, y = boy_ratio)) + 
  geom_line() +
  scale_y_continuous(name="Boy Ratio", limits=c(.475, .55)) +
...
```{r noname} 
min_p_year <- min(present$year)
max_p_year <- max(present$year)
p_colnames <- colnames(present)
p_diff <- max_p_year - min_p_year
```
__Present Data Set Overview__

attribute  | value
-----------|-------
maximum year | `r max_p_year` 
minimum year | `r min_p_year`
column names |  `r p_colnames` 



1.  How do these counts compare to Arbuthnot's? Are they of a similar magnitude?

**Insert your answer here 4**
```{r explore_present }
present <- present %>% 
  mutate(total = boys + girls) |>
  mutate(boy_ratio = boys / total)

min_a_year <- min(arbuthnot$year)
max_a_year <- max(arbuthnot$year)
a_diff <- max_a_year - min_a_year

a_mean <- mean(arbuthnot$boy_ratio)
p_mean <- mean(present$boy_ratio)

a_boys_mean <- mean(arbuthnot$boys)
a_girls_mean <- mean(arbuthnot$girls)

p_boys_mean <- mean(present$boys)
p_girls_mean <- mean(present$girls)

options(scipen = 100, digits = 4)

```
__ Comparing the two Datasets __ 

dataset  | attribute          | attirbute_value
---------|--------------------|----------------
arbuthnot| survey range       | `r a_diff` years
present. | survey range       | `r p_diff` years
arbuthnot| mean (boy ratio)   | `r a_mean`
present  | mean (boy ratio)   | `r p_mean`
arbuthnot| mean (boys/year)   | `r a_boys_mean`
arbuthnot| mean (girls/year)  | `r a_girls_mean`
present  | mean (boys/year)   | `r p_boys_mean`
present  | mean (girls/year)  | `r p_girls_mean`

*At first glance, with one exception, they appear similar. The obvious difference 
is the present dataset is about 150X larger. They are similar in that they both span 
around3/4th of a century, they have boys, girls and year as columns, and there is a 
slight bias toward counting boys in both data sets.*
...
ggp <- ggplot(data = arbuthnot, aes(x = year, y = boy_ratio)) + 
  geom_line() +
  scale_y_continuous(name="Boy Ratio", limits=c(.475, .57)) +
  geom_hline(aes(yintercept=.5), linetype = "dashed", size = 0.5) +
  stat_smooth(method = "lm", formula = y ~ x, geom = "smooth") +
  xlab ("Arbuthnot Data Set")
ggp

ggp2 <- ggplot(data = present, aes(x = year, y = boy_ratio)) + 
  geom_line() +
  scale_y_continuous(name="Boy Ratio", limits=c(.475, .57)) +
  geom_hline(aes(yintercept=.5), linetype = "dashed", size = 0.5) +
  stat_smooth(method = "lm", formula = y ~ x, geom = "smooth") + 
  xlab ("Present Data Set")
ggp2
```

_It does look like more boys were born over time in both studies. It does however also look
like both samples start out slightly more biased toward boys, and have a slight downoard 
slape approaching less biased._ 
...
**Insert your answer here 6**
```{r highest_year}
highest_year <- present %>% 
  arrange(desc(total)) %>% 
  slice(1)

hy = toString(highest_year$year)
ht = toString(highest_year$total)

```
_`r hy` is the year of the study with the greatest birthrate. We were blessed
with `r ht` total children._
