...
plot0 <- ggplot(data = nycflights, aes(x = dep_delay)) +  geom_histogram() +
  labs(title = "Bin = 30/default")
plot1 <- ggplot(data = nycflights, aes(x = dep_delay)) + geom_histogram(binwidth = 15) +
  labs(title = "Bin = 15")
plot2 <- ggplot(data = nycflights, aes(x = dep_delay)) + geom_histogram(binwidth = 150) +
  labs(title = "Bin = 150")
grid.arrange(plot0, plot1, plot2,  ncol = 3)

```

**Insert your answer here: One**

* The binning is different between the plots. It changes the smoothing of the chart dramatically.  

* Are features revealed or hidden by changing binning? A little, not really. Smoothing happens, that's what binning does.  As is, none of these is good enough to tell what the two most important stories with this data might be. The first story is, when some of these planes are late, they are **really** late. Changing binning won't paint that picture. And second, most of most of these planes take off a few minutes early! For that, you'd either want to heavily adjust the labeling of the x axis or show data with the chart like below.


\newpage
```{r showdetails, size="tiny"}
library(dplyr)
library(ggplot2)

flights_grouped <- nycflights %>%
  mutate(bin = cut(dep_delay,
    breaks = seq(
      from = min(dep_delay, na.rm = TRUE),
      to = max(dep_delay, na.rm = TRUE), by = 5),
...
**Insert your answer here: Two**
```{r size="tiny"}
sfo_feb_flights <- nycflights %>%
  filter(dest == "SFO" & month == 2)
print(sprintf("NYC to SFO flights during month 2: %s", nrow(sfo_feb_flights)))
...
**Insert your answer here: Three**

```{r size="tiny"}
library(ggplot2)
library(gridExtra)

plot4 <- ggplot(data = nycflights, aes(x = arr_delay)) +  
  geom_histogram(binwidth = 5) +
  labs(title = "Arrvial Delay") + 
  ylab("planes arriving") +
  xlab("delay in minutes")

plot5 <- ggplot(data = nycflights, aes(x = arr_delay)) +
  geom_boxplot() +
  coord_flip() +
  labs(title = "Boxplot") +
  xlab("delay in minutes") +
...
plot6 <- ggplot(data = nycflights %>%  
    filter(arr_delay < 45 & arr_delay > -40), aes(x = arr_delay )) +
  geom_histogram(binwidth = 5) +
  scale_x_continuous(
    breaks = -45:50,  # tick marks every one unit
    labels = function(x) ifelse(x %% 15 == 0, x, "")  # label every five units
  ) +
...
summarise(mean   = mean(arr_delay), 
          ...
          standard_deviation = sd(arr_delay),
          iqr = IQR(arr_delay),
          record_count = n())
 
```
This data set is close to normally distributed with a slight skew toward arriving a few minutes early. There are also large outliers to consider.
 \newpage
...
sfo_feb_flights |> 
  ...
  summarise(
    ...
    median = median(arr_delay), 
    iqr = IQR(arr_delay),
    standard_deviation = sd(arr_delay, na.rm = TRUE),
    n_flights = n())
```

If the decision were based only based on IQR, United and VX would tie. After adding in standard deviation, United Airlines has the most variable delay. 
...
**Insert your answer here: Five**

The lowest median delay methodology is a little more resistant to outliers and therefore minimizes the potential of delay. The problem is though, you expect one thing, the median experience, and you might end up getting the outlier and very late.

With mean delay, all arrival times are considered including the outliers. This reduces the risk of being on an unexpected late flight, because one probably plans on the flight being at least a little late. This might be good. You expect and plan on being late, and maybe you arrive early. 
...
```
\newpage
**Insert your answer here: Six**
```{r}

best <- head(nycflights %>% 
  group_by(origin) %>% 
  summarise(ot_dep_rate = sum(dep_type == "on time") / n()) %>%
  arrange(desc(ot_dep_rate)), 1) |>
  select("origin")
print(sprintf("Based on time departure alone, I'd select: %s", best))
...
Generally speaking, there is a positive relationship between distance and average speed until about 450Mph.  Also, planes fly slower on shorter flights of under 500 miles. 
...
\newpage
**Insert your answer here: Nine**

```{r }

data_subset <- nycflights %>%
  filter(carrier == "AA" | carrier == "DL" | carrier == "UA") |>
  filter(arr_delay > -100 & arr_delay < 150) |>
  filter(dep_delay > -50 & dep_delay < 200)


ggplot(data_subset, aes(x = dep_delay, y = arr_delay, color = carrier)) +
  geom_point() +
  geom_smooth(color="#362222", se = FALSE) +
  geom_hline(yintercept = 0, color = "black", linetype = "dashed") +
  geom_vline(xintercept = 9, color = "black", linetype = "dashed") +
  scale_x_continuous(breaks = seq(-50, 200, by = 10)) +
  scale_y_continuous(breaks = seq(-100, 150, by = 10)) +
  ylab("Arrival delay in minutes") +
  xlab("Departure delay in minutes")

```

On average, it looks like an NYC departing plane can take off up to about ten minutes late and still arrive on time. 
