...
Please find this code in my [github repo](https://github.com/tonythor/cuny-datascience/blob/develop/data608-vis/story2.qmd).

# Attempt Number 
This is my second submission attempt.  
...
merged_data <- date_seq %>%
  ...
  mutate(inflation_rate = (consumer_price_index - lag(consumer_price_index)) / lag(consumer_price_index) * 100)
...
```{r buildPlotTwo, fig.width=12, fig.height=14, echo=FALSE, message=FALSE, warning=FALSE}
## This is the base R chart. Man what a pain.
## This is the base R chart.

png("nogit_submission2.png", width = 1200, height = 1200, res = 100)

# Set up the layout with outer margins and adjust margins
par(mfrow = c(1, 1), oma = c(6, 4, 6, 4), mar = c(4, 4, 2, 2))

# Sequence of years by five
...
# Plot Unemployment Rate, Federal Funds Rate, and Inflation Rate on the same graph
plot(merged_data$date, merged_data$unemployment_rate, type = "l", col = "red",
     xlab = "Date", ylab = "Rate (%)", cex.lab = 1.2, 
     ylim = range(c(merged_data$unemployment_rate, merged_data$federal_funds_rate, merged_data$inflation_rate), na.rm = TRUE),
     main = "Unemployment, Federal Funds, and Inflation Rates Over Time", xaxt = 'n')

# Add other lines to the same chart
lines(merged_data$date, merged_data$federal_funds_rate, type = "l", col = "green")
lines(merged_data$date, merged_data$inflation_rate, type = "l", col = "blue")

# Add vertical lines for events
abline(v = years, col = "lightgray", lty = "dotted")   # dotted 5-year lines
for (i in seq_along(event_dates)) {
  abline(v = event_dates[i], col = event_colors[i], lty = 2)  # Event lines
}

# Manually add the x-axis ticks for the years variable
axis(1, at = years, labels = format(years, "%Y"), las = 1, cex.axis = 0.8)  # las = 1 ensures horizontal labels

# Add the floating legend without border
par(xpd = NA)  # Allow drawing outside the plot area
legend("topleft", legend = c("Unemployment Rate", "Federal Funds Rate", "Inflation Rate"),
       col = c("red", "green", "blue"), lty = 1, cex = 0.8, bty = "n")  # 'bty = "n"' removes the border

# Add a detached legend for the events (9/11, Financial Crisis, COVID-19)
legend("bottom", inset = c(0, -0.17), legend = c("9/11", "Financial Crisis", "COVID"),
       col = event_colors, lty = 2, horiz = TRUE, cex = 1.3, bty = "n")  # Event legend without a border

text(x = as.Date("2012-12-01"), y = -1.2, labels = "The Fed Proves Effective", pos = 4, cex = 2, col = "blue")


dev.off()  # Writ
```
\begin{center}
\includegraphics[width=0.9\textwidth]{nogit_submission2.png}
\end{center}


```{r build_plot, fig.width=8, fig.height=11, echo=FALSE, message=FALSE, warning=FALSE}
# This is the base R chart on top of chart, my first submission.
